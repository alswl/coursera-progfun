...
object Huffman {
  ...
  def createCodeTree(chars: List[Char]): CodeTree = {
    chars match {
      case Nil => throw new Exception("no node")
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    ...
    def decode0(tree0: CodeTree, bits0: List[Bit]): List[Char] = {
      (bits0, tree0) match {
        case (bit::rest, Fork(_, right, _, _)) if bit == 1 => decode0(right, rest)
        case (_ ::rest, Fork(left, _, _, _)) => decode0(left, rest)
        case (_, Leaf(char, _)) => char :: decode0(tree, bits0)
        case (_, _) => Nil
      }
    }
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    ...
    def encodeChar(tree: CodeTree)(char: Char): List[Bit] = tree match {
      ...
      case Fork(left, right, _, _) if ! chars(left).contains(char) => 1 :: encodeChar(right)(char)
    }
  ...
  def codeBits(table: CodeTable)(char: Char): List[Bit] = {
    table.filter(x => x._1 == char) match {
      ...
      case _ => throw new Exception("no such a char")
    }
  }
  ...
  def convert(tree: CodeTree): CodeTable = {
    ...
    def convert0(tree0: CodeTree, bits: List[Bit]): CodeTable = {
      tree0 match {
        case Leaf(char, _) => List((char, bits))
        case Fork(left, right, _ , _) => convert0(left, bits ++ List(0)) ++
          convert0(right, bits ++ List(1))
      }
    }
